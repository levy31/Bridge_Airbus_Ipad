...
# --- 5. TELECHARGEMENT DES FICHIERS DEVIS ---
...
if uploaded_alpha is not None:
    st.session_state.uploaded_alpha = pd.read_excel(uploaded_alpha, engine='openpyxl')
if uploaded_beta is not None:
    st.session_state.uploaded_beta = pd.read_excel(uploaded_beta, engine='openpyxl')
if uploaded_gamma is not None:
    st.session_state.uploaded_gamma = pd.read_excel(uploaded_gamma, engine='openpyxl')
...
# --- 6. CHARGEMENT DES DEVIS (upload prioritaire, sinon local) ---
raw_dfs = {}
all_wps_list = []

def load_devis(session_key, local_path):
...
for f_name in files_list:
    if f_name == "Devis_Alpha":
        df = load_devis("uploaded_alpha", "Devis_Alpha.xlsx")
    elif f_name == "Devis_Beta":
        df = load_devis("uploaded_beta", "Devis_Beta.xlsx")
    else:
        df = load_devis("uploaded_gamma", "Devis_Gamma.xlsx")
    
    if df is not None:
        if 'Work_Package' not in df.columns or 'Cout_Total' not in df.columns:
            st.error(f"File {f_name} must contain columns 'Work_Package' and 'Cout_Total'.")
            st.stop()
        raw_dfs[f_name] = df
        for wp in df['Work_Package'].unique():
            all_wps_list.append({"System": f_name, "Original WP": wp})
    else:
        st.warning(f"File {f_name} not found. Please upload it or place it in the directory.")

if not raw_dfs:
    st.warning("Please upload or place the three Devis files (Alpha, Beta, Gamma).")
    st.stop()

# --- 7. MAPPING MANAGEMENT (dans la sidebar) ---
...
# --- 8. CHARGEMENT OU CRÉATION DU MAPPING ACTIF ---
...
# --- 9. AFFICHAGE DU MAPPING (dans l'onglet principal) ---
...
for name, df in raw_dfs.items():
    ...
    # Conserver les colonnes heures et taux si elles existent
    if 'Heures' in df_c.columns:
        df_c['Heures'] = df_c['Heures']
    if 'Taux_Horaire' in df_c.columns:
        df_c['Taux_Horaire'] = df_c['Taux_Horaire']
...
with tabs[1]:
    ...
    if len(total_norm_by_system) >= 2:
        ...
        annualized_pct = (slope_per_day * 365 / first_cost) * 100 if first_cost != 0 else 0
        ...
        slope_per_day = slope_per_month = first_cost = annualized_pct = 0

    st.divider()
    st.subheader("5. Individual Work Package drift")

    # Calcul des pentes par WP
    wp_drift_dict = {}
    ...
    if wp_drift_dict:
        ...
        selected_wp = st.selectbox("Choose a Work Package for detailed view:", all_wps, key="wp_selector")
...
# --- ONGLET 3 : COMPETITIVENESS (Rate vs Hours) ---
with tabs[2]:
    st.subheader("6. Rate vs Technical Competitiveness Analysis")
    if 'Heures' in df_global.columns and 'Taux_Horaire' in df_global.columns:
        decomposition_data = []
        for wp in df_global['WP_ST'].unique():
            wp_data = df_global[df_global['WP_ST'] == wp].sort_values('Date')
            if len(wp_data) >= 2:
                first = wp_data.iloc[0]
                last = wp_data.iloc[-1]
                # Coût reconstitué pour garantir cohérence
                cout_first = first['Heures'] * first['Taux_Horaire'] / 1000
                cout_last = last['Heures'] * last['Taux_Horaire'] / 1000
                var_totale_pct = ((cout_last - cout_first) / cout_first) * 100
                # Effets
                effet_taux_brut = ((last['Taux_Horaire'] - first['Taux_Horaire']) * first['Heures'] / 1000) / cout_first * 100
                effet_heures_brut = ((last['Heures'] - first['Heures']) * first['Taux_Horaire'] / 1000) / cout_first * 100
                effet_croise_brut = var_totale_pct - effet_taux_brut - effet_heures_brut
                # Parts relatives
                if abs(var_totale_pct) > 0.01:
                    part_taux = (effet_taux_brut / var_totale_pct) * 100
                    part_heures = (effet_heures_brut / var_totale_pct) * 100
                    part_croise = (effet_croise_brut / var_totale_pct) * 100
                else:
                    part_taux = part_heures = part_croise = 0.0
                # Interprétation basée sur la part des heures
                if part_heures > 60:
                    interprete = "🔴 Strong loss (hours dominate)"
                elif part_heures > 30:
                    interprete = "🟡 Moderate loss"
                elif part_heures < -30:
                    interprete = "🟢 Gain"
                else:
                    interprete = "⚪ Mainly rate-driven"
                decomposition_data.append({
                    'WP': wp,
                    'Total Δ%': f"{var_totale_pct:+.1f}%",
                    'Rate share': f"{part_taux:+.0f}%",
                    'Hours share': f"{part_heures:+.0f}%",
                    'Cross share': f"{part_croise:+.0f}%",
                    'Interpretation': interprete
                })
        if decomposition_data:
            # Stocker dans session_state pour l'onglet IA
            st.session_state.decomposition_data = decomposition_data
            st.dataframe(pd.DataFrame(decomposition_data), use_container_width=True, hide_index=True)
            # Graphique des parts
        ...
        st.info("Hourly rate data not available. Please upload files with 'Heures' and 'Taux_Horaire' columns.")

# --- ONGLET 4 : IA ANALYSIS ---
with tabs[3]:
    st.subheader("7. IA analysis of observed drifts")
    provider_options = []
    if GEMINI_API_KEY:
    ...
    if not provider_options:
        st.error("No IA provider available. Please check your secrets.")
    else:
        ia_provider = st.radio("Choose IA provider", provider_options, horizontal=True, key="ia_provider_drift")
        if st.button("🤖 Analyze drifts with IA", key="btn_drift_analysis"):
            ...
            # Récupérer les données de décomposition stockées
            decomp_data = st.session_state.get('decomposition_data', [])
            decomp_summary = ""
            if decomp_data:
                decomp_summary = "\n\n**Rate vs Hours decomposition (shares of total variation):**\n"
                for item in decomp_data:
                    decomp_summary += f"- {item['WP']}: Total Δ = {item['Total Δ%']}, Rate share = {item['Rate share']}, Hours share = {item['Hours share']} → {item['Interpretation']}\n"
...
3. For each Work Package, distinguish between cost increases due to **rate inflation** (Rate share) and those due to **technical competitiveness loss** (Hours share). Which WPs are losing technical competitiveness?
...
"""
    ...
    st.divider()
    st.subheader("8. Complete Strategic Audit")
...
"""
        # Ajouter la décomposition si disponible
        decomp_data = st.session_state.get('decomposition_data', [])
        if decomp_data:
            summary += "\n\n**Rate vs Hours decomposition:**\n"
            for item in decomp_data:
                summary += f"- {item['WP']}: Total Δ = {item['Total Δ%']}, Rate share = {item['Rate share']}, Hours share = {item['Hours share']}\n"
...
with tabs[4]:
    ...
    if oracle_file:
        try:
            ...
            # Lecture des autres onglets (support anglais/français)
            if "Expected_Global" in sheet_names or "Attendus_Global" in sheet_names:
                sheet_global = "Expected_Global" if "Expected_Global" in sheet_names else "Attendus_Global"
                df_oracle_global = pd.read_excel(oracle_file, sheet_name=sheet_global, engine='openpyxl')
                if "Expected_WP" in sheet_names or "Attendus_WP" in sheet_names:
                    sheet_wp = "Expected_WP" if "Expected_WP" in sheet_names else "Attendus_WP"
                    df_oracle_wp = pd.read_excel(oracle_file, sheet_name=sheet_wp, engine='openpyxl')
                    if "Expected_IA_Analysis" in sheet_names or "Analyse_IA_attendue" in sheet_names:
                        sheet_ia = "Expected_IA_Analysis" if "Expected_IA_Analysis" in sheet_names else "Analyse_IA_attendue"
                        expected_analysis = pd.read_excel(oracle_file, sheet_name=sheet_ia, engine='openpyxl').iloc[0,0]
                    else:
                        expected_analysis = "No IA analysis provided."
                else:
                    st.error("Oracle must contain a WP sheet.")
                    st.stop()
            else:
                st.error("Oracle must contain a global sheet.")
                st.stop()

            st.subheader("🔬 Comparison with expected scenario (Oracle)")
            # Global comparison
            st.markdown("#### Global comparison")
            computed_global = ((total_norm_by_system['Normalized_Cost'].iloc[-1] / total_norm_by_system['Normalized_Cost'].iloc[0]) - 1) * 100 if len(total_norm_by_system)>=2 else 0
            # Trouver la colonne de métrique
            if 'Metric' in df_oracle_global.columns:
                metric_col = 'Metric'
                value_col = 'Value'
            else:
                metric_col = 'Metrique'
                value_col = 'Valeur'
            expected_global_str = df_oracle_global[df_oracle_global[metric_col] == 'Normalized_global_drift_%'][value_col].values[0]
            expected_global = float(str(expected_global_str).replace('%','').replace('+','').strip())
            ...
            wp_col = 'WP' if 'WP' in df_oracle_wp.columns else 'WP'
            exp_col = 'Expected_normalized_drift_%' if 'Expected_normalized_drift_%' in df_oracle_wp.columns else 'Dérive_normalisee_%_attendue'
            alert_col = 'Alert' if 'Alert' in df_oracle_wp.columns else 'Alerte'
            comment_col = 'Expected_comment' if 'Expected_comment' in df_oracle_wp.columns else 'Commentaire_attendu'
            comparisons = []
            for _, row in df_oracle_wp.iterrows():
                wp_oracle = row[wp_col]
                expected_drift = row[exp_col]
                expected_alert = row[alert_col]
                comment = row[comment_col]
                # Chercher dans df_wp_drift
                found = None
                for wp_calc in df_wp_drift['Work Package']:
                    if wp_calc.strip().lower() == wp_oracle.strip().lower():
                        ...
                        break
                if found:
                    computed = float(df_wp_drift[df_wp_drift['Work Package']==found]['Annualized drift (%)'].iloc[0])
                    if expected_drift != 'N/A':
                        expected_val = float(str(expected_drift).replace('%','').replace('+','').strip())
                ...
                comparisons.append({
                    ...
                    'Computed': f"{computed:.1f}%" if computed else "N/A",
                    'Difference': f"{diff:+.1f}" if diff else "N/A",
            ...
            if 'ai_audit' in st.session_state:
                ...
                col_ia2.warning("Please run the full strategic audit first (section 8).")
        except Exception as e:
            st.error(f"Error reading oracle: {e}")
    else:
        st.info("Load an oracle file to validate results.")

# --- 9. AUDIT GUIDE (optionnel, en bas) ---
st.divider()
st.subheader("📚 Audit Guide")
st.markdown("""
* **Snapshots**: Raw and normalized cost views, with bridge charts.
* **Drifts**: Global trend and per-WP slopes.
* **Competitiveness**: Decomposition of cost variations into rate (inflation) and hours (technical) effects.
* **IA Analysis**: Two levels of AI-powered commentary, now including rate vs hours decomposition.
