...
if oracle_file:
    ...
    # Obtenir la liste des onglets du fichier
    sheet_names = pd.ExcelFile(oracle_file, engine='openpyxl').sheet_names

    # --- Lecture des onglets (supporte anglais et français) ---
    try:
        # Onglet global
        if "Expected_Global" in sheet_names:
            df_oracle_global = pd.read_excel(oracle_file, sheet_name="Expected_Global", engine='openpyxl')
        elif "Attendus_Global" in sheet_names:
            df_oracle_global = pd.read_excel(oracle_file, sheet_name="Attendus_Global", engine='openpyxl')
        else:
            st.error("Oracle file must contain a sheet named 'Expected_Global' or 'Attendus_Global'")
            st.stop()

        # Onglet WP
        if "Expected_WP" in sheet_names:
            df_oracle_wp = pd.read_excel(oracle_file, sheet_name="Expected_WP", engine='openpyxl')
        elif "Attendus_WP" in sheet_names:
            df_oracle_wp = pd.read_excel(oracle_file, sheet_name="Attendus_WP", engine='openpyxl')
        else:
            st.error("Oracle file must contain a sheet named 'Expected_WP' or 'Attendus_WP'")
            st.stop()

        # Onglet analyse IA
        if "Expected_IA_Analysis" in sheet_names:
            expected_analysis = pd.read_excel(oracle_file, sheet_name="Expected_IA_Analysis", engine='openpyxl').iloc[0,0]
        elif "Analyse_IA_attendue" in sheet_names:
            expected_analysis = pd.read_excel(oracle_file, sheet_name="Analyse_IA_attendue", engine='openpyxl').iloc[0,0]
        else:
            st.error("Oracle file must contain a sheet named 'Expected_IA_Analysis' or 'Analyse_IA_attendue'")
            st.stop()
        ...
        if len(total_norm_by_system) >= 2:
            ...
            computed_global = 0
        # Trouver la colonne de la métrique (peut s'appeler 'Metric' ou 'Metrique')
        if 'Metric' in df_oracle_global.columns:
            metric_col = 'Metric'
            value_col = 'Value'
        elif 'Metrique' in df_oracle_global.columns:
            metric_col = 'Metrique'
            value_col = 'Valeur'
        else:
            st.error("Global sheet must contain 'Metric'/'Value' or 'Metrique'/'Valeur' columns")
            st.stop()
        
        expected_global_str = df_oracle_global[df_oracle_global[metric_col] == 'Normalized_global_drift_%'][value_col].values[0]
        expected_global_clean = str(expected_global_str).replace('%', '').replace('+', '').strip()
        expected_global = float(expected_global_clean)
        ...
        # Identifier les colonnes (anglais ou français)
        wp_col = 'WP' if 'WP' in df_oracle_wp.columns else 'WP'  # par défaut
        expected_col = 'Expected_normalized_drift_%' if 'Expected_normalized_drift_%' in df_oracle_wp.columns else 'Dérive_normalisee_%_attendue'
        ...
        for _, row in df_oracle_wp.iterrows():
            ...
            expected_drift = row[expected_col]
            ...
            if found_wp is not None:
                ...
                if expected_drift != 'N/A':
                    expected_drift_clean = str(expected_drift).replace('%', '').replace('+', '').strip()
                    expected_val = float(expected_drift_clean)
        ...
        st.info("Make sure the Excel file contains the required sheets with appropriate column names.")

# ========== IA DRIFT ANALYSIS ==========
st.divider()
st.subheader("6. IA analysis of observed drifts")

# Sélecteur du fournisseur IA
provider_options = []
if GEMINI_API_KEY:
...
* **IA analysis**: Two buttons – one for focused drift analysis, one for a complete strategic audit including data and comments. Choose your preferred IA provider above the buttons.
