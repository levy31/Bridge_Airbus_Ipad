...
# --- 2. INITIALIZE DATES IN SESSION STATE ---
if "dates" not in st.session_state:
    # New default dates: Alpha 10/06/2026, Beta 20/03/2024, Gamma 15/01/2022
...
# Shortcut for the dates dictionary used in the rest of the code
...
# --- 6. LOAD OR CREATE ACTIVE MAPPING ---
...
# --- 8. THE 6 SNAPSHOTS WITH BASE/TARGET SELECTORS ---
st.divider()
...
with c6:
    # Selectors for normalized bridge
...
if len(total_norm_by_system) >= 2:
    ...
    fig_global_drift.add_trace(go.Scatter(x=trend_dates, y=y_pred, mode='lines', line=dict(dash='dash', color='red'),
                                           name=f'Linear trend (slope = {slope:.2f} €/day)'))
...
if wp_drift_dict:
    ...
    if wp_info['pente'] is not None:
        ...
        fig_wp.add_trace(go.Scatter(x=trend_dates_wp, y=y_pred_wp, mode='lines', line=dict(dash='dash', color='red'),
                                     name=f'Trend (slope = {slope_wp_indiv:.2f} €/day)'))
        st.caption(f"Annualized drift: {wp_info['annual_pct']:.1f} % / year")
    else:
...
if st.button("🤖 Analyze drifts with IA", key="btn_drift_analysis"):
    try:
        ...
        global_trend = f"global slope = {slope:.2f} €/day, i.e. {annualized_pct:.1f}% per year (initial total cost = {first_cost:.2f} €)"
        wp_trends = []
        for wp, vals in wp_drift_dict.items():
            if vals['pente'] is not None:
                wp_trends.append(f"- {wp}: slope = {vals['pente']:.2f} €/day, annualized = {vals['annual_pct']:.1f}% per year")
            else:
...
* **Per WP drift**: Table of individual slopes (€/day and annualized %) for each Work Package, and individual graph with selector (all WPs listed).
* **IA analysis**: Two buttons – one for focused drift analysis, one for a complete strategic audit including data and comments.
