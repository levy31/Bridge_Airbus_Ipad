...
import numpy as np
import requests  # Pour appeler DeepSeek API
...
# Gestion des clés API (Gemini et DeepSeek)
if "GEMINI_API_KEY" in st.secrets:
    genai.configure(api_key=st.secrets["GEMINI_API_KEY"])
else:
    st.warning("Gemini API key missing. DeepSeek will be used if available.")

DEEPSEEK_API_KEY = st.secrets.get("DEEPSEEK_API_KEY", None)

# --- 2. FONCTION D'APPEL DEEPSEEK (fallback) ---
def call_deepseek_api(prompt):
    """Appelle l'API DeepSeek en utilisant la clé stockée dans les secrets."""
    if not DEEPSEEK_API_KEY:
        st.error("DeepSeek API key is missing in secrets.")
        return None
    try:
        headers = {
...
# --- 7. LOAD OR CREATE ACTIVE MAPPING ---
...
def draw_bridge(pivot_df, base_sys, target_sys):
    """Generate a waterfall chart between two systems."""
...
# --- 9. THE 6 SNAPSHOTS WITH BASE/TARGET SELECTORS ---
st.divider()
...
if len(total_norm_by_system) >= 2:
    ...
    slope_per_month = slope_per_day * 30.44  # average days per month
    ...
    slope_per_day, slope_per_month, slope_per_year, first_cost, annualized_pct = 0, 0, 0, 0, 0
...
for wp in df_global['WP_ST'].unique():
    ...
    if len(wp_data) >= 2:
        ...
        slope_per_year_wp = slope_per_day_wp * 365
...
for wp, vals in wp_drift_dict.items():
    if vals['pente_jour'] is not None:
        wp_drift_list.append({
            ...
            'Slope (€/year)': round(vals['pente_an'], 2),
...
if st.button("🤖 Analyze drifts with IA", key="btn_drift_analysis"):
    ...
    **Detailed data** (optional, for reference):
    ...
    with st.spinner("IA analyzing drifts..."):
        # Try Gemini first
        ...
        if "GEMINI_API_KEY" in st.secrets:
            try:
                ...
                if "429" in str(e) or "quota" in str(e).lower():
                    st.warning("Gemini quota exceeded. Switching to DeepSeek...")
                else:
        ...
        if not gemini_success:
            if DEEPSEEK_API_KEY:
                ...
                if response_text:
                    ...
                    st.error("DeepSeek also failed. Please check your API key or try again later.")
            else:
                st.error("No DeepSeek API key configured. Please add DEEPSEEK_API_KEY to secrets.")
...
if st.button("🧠 Execute Full Strategic Audit (including data and drifts)", key="btn_full_audit"):
    ...
    with st.spinner("Senior Airbus audit in progress..."):
        # Try Gemini first
        gemini_success = False
        if "GEMINI_API_KEY" in st.secrets:
            try:
...
* **Per WP drift**: Table of individual slopes (€/day, €/month, €/year and annualized %) for each Work Package, and individual graph with selector (all WPs listed).
* **IA analysis**: Two buttons – one for focused drift analysis, one for a complete strategic audit including data and comments. Automatically falls back to DeepSeek if Gemini quota is exceeded.
